...
class VideoInformation:
    def __init__(self, link, resolution="720p"):
        ...
        try:
            self.video_object = self.video_object.streams.get_by_resolution(resolution)
        except: # if had problem downloading it will download the highest resolution video
            self.video_object = self.video_object.streams.get_highest_resolution()

global current_video_info
current_video_info = {}
def Download():#link, file_type="MP4", resolution = "720p"):
    global current_video_info
    link = url_entry.get()
    print(link)
...
# Download button
download_button = tk.Button(root, text="Download", command=wrapper_func)#Download)  # Add the command to your download function
